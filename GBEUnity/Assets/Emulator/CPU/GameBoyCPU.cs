...
namespace Emulator.CPU
{
    public class GameBoyCPU
    {
        ...
        public bool Halted;
        public bool Stopped;
        public static byte Ticks; //Cycle timers;
        public static bool TimerRunning;
        public static byte TimerCounter;
        public static byte TimerModulo;
        ...
        public void Step()
        {
            ...
            if (!Halted)
            {
                opCode = _memory.ReadByte(PC); //Procesor pobier jakie instrukcje ma zrobić z własnej pamieci
                if (StopCounting)
                {
                    StopCounting = false;
                }
        ...
        private void Load(ref byte a, byte b)
        {
            a = b;
            Ticks += 4;
        }
        ...
        {
            A = _memory.ReadByte((ushort)(0xFF00 | _memory.ReadByte(PC++)));
            Ticks += 19;
        }
        ...
        {
            ushort offset = _memory.ReadByte(PC++);
            if (offset > 0x7F)
            {
                offset -= 256;
            }
            offset += SP;
            H = (byte)(0xFF & (offset >> 8));
            L = (byte)(0xFF & offset);
            Ticks += 20;
        }
        ...
        {
            ushort value = _memory.ReadByte(PC++);
            if (value > 0x7F)
            {
                value -= 256;
            }
        ...
        {
            _memory.WriteByte((ushort)(0xFF00 | _memory.ReadByte(PC++)), A);
            Ticks += 19;
        }

        private void Swap(byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
        ...
        private void Swap(ref byte r)
        {
            r = (byte)(0xFF & ((r << 4) | (r >> 4)));
            Ticks += 8;
        }

        private void SetBit(byte bit, byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var value = _memory.ReadByte(address);
        ...
        private void SetBit(byte bit, ref byte a)
        {
            a |= (byte)(1 << bit);
            Ticks += 8;
        }

        private void ResetBit(byte bit, byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var value = _memory.ReadByte(address);
        ...
        private void ResetBit(byte bit, ref byte a)
        {
            switch (bit)
            {
                case 0: // 1111 1110
        ...
        private void TestBit(byte bit, byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var value = _memory.ReadByte(address);
            TestBit(bit, value);
        ...
        private void TestBit(byte bit, byte a)
        {
            FZ = (a & (1 << bit)) == 0;
            FN = false;
            FH = true;
        ...
        private void Jump(byte ah, byte al)
        {
            PC = (ushort)((ah << 8) | al);
            Ticks += 4;
        }
        ...
        private void Pop(ref byte rh, ref byte rl)
        {
        ...
        private void Push(byte rh, byte rl)
        {
        ...
        private void Or(byte addressHigh, byte addressLow)
        {
            Or(_memory.ReadByte((ushort) ((ushort)(addressHigh << 8) | addressLow)));
            Ticks += 3;
        }
        ...
        {
            Xor(_memory.ReadByte((ushort)((ushort)(addressHigh << 8) | addressLow)));
        }
        ...
        private void And(byte addressHigh, byte addressLow)
        {
            And(_memory.ReadByte((ushort) ((addressHigh << 8) | addressLow)));
            Ticks += 3;
        }
        ...
        private void LoadImmediateIntoMemory(byte ah, byte al)
        {
            _memory.WriteByte((ushort) ((ah << 8) | al), _memory.ReadByte(PC++));
            Ticks += 10;
        }
        ...
        {
            ushort relativeAddress = _memory.ReadByte(PC++);
        ...
        private void Add(byte addressHigh, byte addressLow)
        {
            Add(_memory.ReadByte((ushort) ((addressHigh << 8) | addressLow)));
            Ticks += 3;
        }
        ...
        private void AddWithCarry(byte addressHigh, byte addressLow)
        {
            AddWithCarry(_memory.ReadByte((ushort) ((addressHigh << 8) | addressLow)));
            Ticks += 3;
        }
        ...
        private void SubWithBorrow(byte ah, byte al)
        {
            SubWithBorrow(_memory.ReadByte((ushort) ((ah << 8) | al)));
            Ticks += 3;
        }
        ...
        private void SubWithBorrow(byte b)
        {
            if (FC)
            {
                Sub((byte) (b + 1));
        ...
        private void Sub(byte ah, byte al)
        {
            Sub(_memory.ReadByte((ushort) ((ah << 8) | al)));
            Ticks += 3;
        }

        private void Compare(byte ah, byte al)
        {
            Compare(_memory.ReadByte((ushort) ((ah << 8) | al)));
            Ticks += 3;
        }
        ...
        {
            Add(ref H, ref L, (byte)(SP >> 8), (byte)(SP & 0xFF));
        }

        private void Add(ref byte ah, ref byte al, byte bh, byte bl)
        {
            al += bl;
        ...
        private void ShiftLeft(byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var value = _memory.ReadByte(address);
            ShiftLeft(ref value);
        ...
        private void ShiftLeft(ref byte a)
        {
            FC = a > 0x7F;
            a = (byte) (0xFF & (a << 1));
            FZ = a == 0;
        ...
        private void UnsignedShiftRight(byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var value = _memory.ReadByte(address);
        ...
        private void UnsignedShiftRight(ref byte a)
        {
            FC = (a & 0x01) == 1;
            a >>= 1;
            FZ = a == 0;
        ...
        private void SignedShiftRight(byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var value = _memory.ReadByte(address);
        ...
        private void SignedShiftRight(ref byte a)
        {
            FC = (a & 0x01) == 1;
            a = (byte)((a & 0x80) | (a >> 1));
            FZ = a == 0;
        ...
        private void RotateALeftThroughCarry()
        {
            var highBit = FC ? 1 : 0;
            FC = A > 0x7F;
            A = (byte) (((A << 1) & 0xFF) | highBit);
            FN = false;
            FH = false;
            Ticks += 4;
        }

        private void RotateRight(ref byte a)
        {
            var lowBit = a & 0x01;
            FC = lowBit == 1;
            a = (byte) ((a >> 1) | (lowBit << 7));
        ...
        private void RotateRightThroughCarry(byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var value = _memory.ReadByte(address);
        ...
        private void RotateRightThroughCarry(ref byte a)
        {
        ...
        private void RotateLeftThroughCarry(byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var value = _memory.ReadByte(address);
        ...
        private void RotateLeftThroughCarry(ref byte a)
        {
            var highBit = FC ? 1 : 0;
            FC = (a >> 7) == 1;
            a = (byte) (((a << 1) & 0xFF) | highBit);
        ...
        private void RotateLeft(byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var value = _memory.ReadByte(address);
        ...
        private void RotateRight(byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var value = _memory.ReadByte(address);
        ...
        private void RotateLeft(ref byte a)
        {
            var highBit = a >> 7;
            FC = highBit == 1;
            a = (byte) (((a << 1) & 0xFF) | highBit);
        ...
        private void LoadFromImmediateAddress(ref byte r)
        {
        ...
        private void LoadImmediate(ref byte r)
        {
            r = _memory.ReadByte(PC++);
            Ticks += 7;
        }

        private void LoadImmediateWord(ref ushort r)
        {
        ...
        private void LoadImmediate(ref byte rh, ref byte rl)
        {
        ...
        private void ReadByte(ref byte r, byte ah, byte al)
        {
            r = _memory.ReadByte((ushort) ((ah << 8) | al));
            Ticks += 7;
        }

        private void WriteByte(byte ah, byte al, byte value)
        {
            _memory.WriteByte((ushort) ((ah << 8) | al), value);
            Ticks += 7;
        }

        private void WriteWordToImmediateAddress(ushort value)
        {
        ...
        private void Decrement(ref byte rh, ref byte rl)
        {
            if (rl == 0)
            {
                rh = (byte) (0xFF & (rh - 1));
        ...
        private void IncrementWord(ref ushort r)
        {
            if (r == 0xFFFF)
            {
                r = 0;
        ...
        private void DecrementWord(ref ushort r)
        {
            if (r == 0)
            {
                r = 0xFFFF;
        ...
        private void DecrementMemory(byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
        ...
        private void IncrementMemory(byte ah, byte al)
        {
            var address = (ushort)((ah << 8) | al);
            var r = _memory.ReadByte(address);
            Increment(ref r);
        ...
        private void Increment(ref byte rh, ref byte rl)
        {
            if (rl == 255)
            {
                rh = (byte) (0xFF & (rh + 1));
        ...
        private void Increment(ref byte r)
        {
            FH = (r & 0x0F) == 0x0F;
            r++;
            r &= 0xFF;
        ...
        private void Decrement(ref byte r)
        {
            FH = (r & 0x0F) == 0x00;
            r--;
            r &= 0xFF;
