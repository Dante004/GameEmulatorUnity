...
namespace Emulator.Debugger {
	...
	public class VRAMViewerWindow : EditorWindow {
		...
		void ShowBGMap()
		{
			...
			var addr = emu.ppu.LcdcBGTileMap == 0 ? 0x9800 : 0x9C00;
			for (int i = 0; i < 32; i++) {
				for (int j = 0; j < 32; j++) {

					int n = (int)emu.memory.Read((ushort)(addr + i * 32 + j));
					if (emu.ppu.LcdcBGWindowTileData == 0) {
						if (n > 127) {
							n -= 0x100;
						}
						n = 256 + n;
		...
		void UpdateTilesDict()
		{
			...
			for (int n = 0; n < TOTAL_TILES; n++) {
				...
				for (int i = 0; i < 8; i++) {
					byte b1 = emu.memory.Read((ushort)(addr + i * 2));
					byte b2 = emu.memory.Read((ushort)(addr + i * 2 + 1));
